...
@Suite("Rev57 deep comprehensive training") struct Rev57DeepTrainingTests {
 ...
 @Test func roundTrip() throws {let matches=try runOnLargeStack{()->Bool in let x=EERev57DeepTrainingSystem();let d=try JSONEncoder().encode(x);let y=try JSONDecoder().decode(EERev57DeepTrainingSystem.self,from:d);return y.training.catalog.skills.count==x.training.catalog.skills.count};#expect(matches)}
}
...
@Suite("Rev70 end-to-end qualification jobs") struct Rev70QualificationTests {
 ...
 @Test func persistenceRoundTrip() throws { let matches=try runOnLargeStack{()->Bool in let x=EERev70EndToEndQualificationJobs(); let d=try JSONEncoder().encode(x); let y=try JSONDecoder().decode(EERev70EndToEndQualificationJobs.self,from:d); return y.active.job.id == x.active.job.id}; #expect(matches) }
}
