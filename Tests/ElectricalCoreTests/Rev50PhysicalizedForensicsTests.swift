...
@Suite("Rev50 physicalized forensics") struct Rev50PhysicalizedForensicsTests {
 ...
 @Test func roundTrip() throws {let m=EERev50PhysicalizedForensics();let d=try JSONEncoder().encode(m);let r=try JSONDecoder().decode(EERev50PhysicalizedForensics.self,from:d);#expect(r.geometry.paths.count == m.geometry.paths.count)}
}
